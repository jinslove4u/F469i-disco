...
//  if (Controller.available())
  if (Serial.available())
  {
    RcvData = Serial.read();
           ...
           gb.display.setCursor(250, 100);
           gb.display.println("receve DATA");
...
const int _pinkyTargetOffset[] = { 4,0,0,4,-4,0,-4,4 }; // Includes pinky target bug
   ...
   for (byte tmpY=0;tmpY<8;tmpY++)
   {
            ...
            for (byte tmpX=0;tmpX<8;tmpX++) {
               word next_color=(word)_paletteW[indexmap[++i]];
                ...
                if (color >= 0xa0 ) gb.display.drawPixel(x+tmpX, y+tmpY, GREEN);
                if (color >= 0x40 && color <= 0x9f ) gb.display.drawPixel(x+tmpX, y+tmpY, RED);
//                if (color >= 0xff && color <= 0xf0  ) gb.display.drawPixel(x+tmpX, y+tmpY, PINK);
//                if (color >= 0xff && color <= 0xf0  ) gb.display.drawPixel(x+tmpX, y+tmpY, BROWN);
                if (color >= 0x01 && color <= 0x3f  ) gb.display.drawPixel(x+tmpX, y+tmpY, YELLOW);
//                if (color >= 0xff && color <= 0xf0  ) gb.display.drawPixel(x+tmpX, y+tmpY, GREEN);
//                if (color >= 0xff && color <= 0xf0  ) gb.display.drawPixel(x+tmpX, y+tmpY, BLUE);
//                if (color >= 0xff && color <= 0xf0  ) gb.display.drawPixel(x+tmpX, y+tmpY, BLACK);
                if (color <= 0x00 ) gb.display.drawPixel(x+tmpX, y+tmpY, BLACK);
                
//                 gb.display.drawPixel(x+tmpX, y+tmpY, BLACK);
                 gb.update();
                 color=next_color;
                 width=0;
               width++;
            ...
            if (InPen(s->cx,s->cy))
            {
                ...
                else
                {
                    ...
                    switch (s->who)
                    {
                        case PINKY:
                            {
                                const int* pto = _pinkyTargetOffset + ((pacman->dir-1)<<1);
...
///        for (byte y = 34; y<36; y++)
    ...
    void Step()
    {
        ...
        if (GAMEPAUSED && but_START && DEMO==0) {
           ...
           gb.display.setColor(BLUE);
           gb.display.setCursor(250, 60);
           gb.display.println("RESTART Pacman");
        }
        ...
        if (but_SELECT) {
           ...
           gb.display.setColor(BLUE);
           gb.display.setCursor(250, 60);
           gb.display.println("DEMO Pacman");           
        }
